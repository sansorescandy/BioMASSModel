		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					frame.setTitle("Multiagent-based Marine Ecosystem Simulation");
					frame.pack();
...
*/					
					while(true){
						/*if (getShouldStopRunThread())
							break;*/
						//t1=System.currentTimeMillis();
						for(int l=0;l<steploops && !modelstop;l++) {
							for(int j=0;j<stepstable[l][0];j++){
								modelstop = model.step(steps, stepstable[l][1]) || getShouldStopRunThread() || steps >= getWhenShouldEnd(); 
								steps+=stepstable[l][1];
								if (modelstop) {
									model.stop();
		                            break;
								}
...
*/						if (getShouldStopRunThread() || steps >= getWhenShouldEnd() || modelstop) {
							model.stop();
                            break;
						}
